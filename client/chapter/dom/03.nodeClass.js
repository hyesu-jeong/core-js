...
// 사용예시
if (first.tagName === 'SPAN') { // 무조건 대문자로만
  // condition
}
...
first.textContent = '30% 파격 세일' // settter
...
// console.log('hedden(getter): ',first.hidden);
// console.log('hedden(setter): ',first.hidden = true);
...
/* ------------- gsap 코드 활용해서 부드러운 애니메이션 만들기 ------------- */
// gsap 활용 주의사항 : 애니메이션 자체를 gsap이 처리해주고 있다보니 transition으로 커스텀 하면 이상해진다.

gsap.to('h1', {
  opacity: 0,
  duration: 0.5,
  repeat: -1,      // 무한 반복
  yoyo: true,      // 왔다갔다 양방향 동일하게 적용
  delay: 1,
  ease: 'power2.inOut',
})